...
(eval-when (:compile-toplevel :load-toplevel :execute)
  ...
  (deftype body () '(simple-array f64 (7)))
...
;; Type declarations
(deftype position-deltas () `(simple-array f64 (,+ROUNDED-INTERACTIONS-COUNT+)))
(deftype magnitudes      () `(simple-array f64 (,+ROUNDED-INTERACTIONS-COUNT+)))
...
(declaim (ftype (function (list fixnum) null) advance))
(defun advance (system n)
  (let ((position-deltas-x (make-array +ROUNDED-INTERACTIONS-COUNT+
			 :element-type 'f64 :initial-element 1.0d0))
	(position-deltas-y (make-array +ROUNDED-INTERACTIONS-COUNT+
			 :element-type 'f64 :initial-element 1.0d0))
	(position-deltas-z (make-array +ROUNDED-INTERACTIONS-COUNT+
			 :element-type 'f64 :initial-element 1.0d0))
        (magnitudes (make-array +ROUNDED-INTERACTIONS-COUNT+
			        :element-type 'f64 :initial-element 1.0d0)))
    (declare (position-deltas position-deltas-x position-deltas-y position-deltas-z)
             (magnitudes magnitudes))
    (loop repeat n
          do ;; Calculate the position_Deltas between the bodies for each
	     ;; interaction.
	     (loop with i of-type fixnum = 0 for (a . rest) on system 
		   do (dolist (b rest)
			;; (declare (type position_deltas pdx pdy pdz))
			(setf (aref position-deltas-x i) (- (x a) (x b))
			      (aref position-deltas-y i) (- (y a) (y b))
			      (aref position-deltas-z i) (- (z a) (z b)))
			(incf i)))
	     ...
	     (loop for i of-type fixnum below +ROUNDED-INTERACTIONS-COUNT+ by 4
	           do (let* ((pdx (f64.4-aref position-deltas-x i))
	        	     (pdy (f64.4-aref position-deltas-y i))
	        	     (pdz (f64.4-aref position-deltas-z i))
	     ...
	     (loop with i of-type fixnum = 0 for (a . rest) on system
		   do (dolist (b rest)
			(let* ((pdx-i (aref position-deltas-x i))
			       (pdy-i (aref position-deltas-y i))
			       (pdz-i (aref position-deltas-z i))
			       ...
			       (mass*mag-b (f64* (mass b) mag-i)))
                          (declare (type f64 pdx-i pdy-i pdz-i mag-i
                                         mass*mag-a mass*mag-b))
...
(declaim (ftype (function (list) null) output-energy))
(defun output-energy (system)
    (let ((e 0.0d0))
      (declare (type f64 e))
	...
	;; Add the kinetic energy for each body.
	  ...
	  ;; Add the potential energy between this body and every other bodies
          (let* ((dx (f64- (x a) (x b)))
            ...
            (declare (type f64 dx dy dz dist))
...
(defun scale-bodies (system scale)
  (dolist (p system)
    ...
    (setf (mass p) (f64* (mass p) (f64* scale scale))
	  (vx p)   (f64* (vx p) scale)
	  (vy p)   (f64* (vy p) scale)
	  (vz p)   (f64* (vz p) scale))) nil)

(declaim (ftype (function (fixnum) null) nbody))
(defun nbody (n)
  (let ((system *system*))
    (offset-momentum system)         ;; Output initial energy of the system
    (output-energy system)           ;; Scale bodies to use time step of 1.0d0
    (scale-bodies system +DT+)       ;; Advance system with time step of 1.0d0
    (advance system n)               ;; Advance system n times
    (scale-bodies system +RECIP_DT+) ;; Rescale bodies
    (output-energy system)))         ;; Output final energy of the system
